...
function renderSettings() {
  ...
  document.getElementById('confirmDeletions').checked = settings.confirmDeletions;
    ...
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          // Restore batches
          if (backup.batches) {
            localStorage.setItem('attendanceBatches', JSON.stringify(backup.batches));
          }
...
function exportAllData() {
  ...
  if (format === 'json') {
    ...
    filename = `attendance_export_${new Date().toISOString().split('T')[0]}.json`;
    ...
    filename = `attendance_export_${new Date().toISOString().split('T')[0]}.txt`;
    mimeType = 'text/plain';
  }
...
function convertToCSV(batches) {
  ...
  Object.keys(batches).forEach(batchName => {
    ...
    batch.groups.Group_1.forEach(participant => {
      ...
      const type = participant.includes('(RP)') ? 'RP' : participant.includes('(C)') ? 'Coordinator' : 'Regular';
...
function addToBackupHistory(backup) {
  ...
  history.unshift({
    ...
    filename: `attendance_backup_${new Date().toISOString().split('T')[0]}_${new Date().getTime()}.json`
  });
